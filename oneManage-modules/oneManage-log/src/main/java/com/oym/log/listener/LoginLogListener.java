...
public class LoginLogListener {
    ...
    @KafkaListener(topics = "login_log_topic", groupId = "${kafka.consumer.group.id:oneManage-log}")
    public void consume(ConsumerRecord<?, ?> record, Acknowledgment ack, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
        ...
        if (message.isPresent()) {
            try {
                ...
                LoginMessage loginMessage = GsonUtil.fromJson(String.valueOf(msg), LoginMessage.class);
                LoginLogDTO loginlogDTO = LogTransform.INS.transfer(loginMessage);
                saveLoginLog(loginlogDTO);
                log.info("login_log_topic consumer: Topic:{},Message:{}", topic, loginMessage.toString());
            } catch (Exception e) {
    ...
    private void saveLoginLog(LoginLogDTO loginlogDTO) {
        try {
            elasticsearchTemplate.save(loginlogDTO);
        } catch (Exception e) {
